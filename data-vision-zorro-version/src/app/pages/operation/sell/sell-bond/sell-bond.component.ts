...
@Component({
  selector: 'app-sell-bond',
  templateUrl: './sell-bond.component.html',
  styleUrls: ['./sell-bond.component.scss']
})
export class SellBondComponent {
  loading: boolean;
  pageIndex: number;
  pageSize: number;
  total: number;
  ...
  showModal1(data: any): void {
    ...
    this.getBondHistory(encodeURIComponent(data.bondName));
  ...
  public getHostData(id: number, date: string) {
    this.loading = true;
    this.tableService.getUserHostBonds$(id, date).subscribe(
      data => {
        console.log(data);
  ...
  handleOk(): void {
    ...
    this.sell_post_data.productCode = this.selected_data.bondCode;
    this.sell_post_data.productType = 'BOND';
