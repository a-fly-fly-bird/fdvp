...
export class SellFundComponent {
  loading: boolean;
  pageIndex: number;
  pageSize: number;
  total: number;
  ...
  showModal1(data: any): void {
    ...
    this.getFundHistory(data.fundName);
  ...
  public getHostData(id: number, date: string) {
    this.loading = true;
    this.tableService.getUserHostFunds$(id, date).subscribe(
      data => {
        console.log(data);
  ...
  handleOk(): void {
    ...
    this.sell_post_data.productCode = this.selected_data.fundName;
    this.sell_post_data.productType = 'FUND';
