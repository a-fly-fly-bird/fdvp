...
export class TableComponent implements OnInit {
  ...
  onChange(result: Date): void {
    ...
    this.tableService.getTableData$(this.pageIndex, this.pageSize, dateString).subscribe(
      data => {
        console.log(data);
  ...
  handleOk(): void {
    ...
    this.buy_post_data.productCode = this.selected_data.stockCode;
    this.buy_post_data.productType = 'STOCK';
    ...
    this.postBuyData(this.buy_post_data);
    setTimeout(() => {
      this.isVisible = false;
      this.isOkLoading = false;
      this.route.navigate(['/dashboard/operation/sell_stock']);
    }, 1000);
  }
  ...
  public postBuyData(data: SellPostData) {
    this.tableService.postUserBuyData$(data).subscribe(
      (x) => {
        console.log(x);
        this.loading = true;
        // this.getHostData(1, this.date);
        this.tableService.getTableData$(this.pageIndex, this.pageSize, data.transactionDate).subscribe(
          data => {
            console.log(data);
            this.total = data.totalElements;
            this.listOfAssetsData = data.content;
  ...
  ngOnInit(): void {
    ...
    this.buy_post_data.userId = this.globalService.getUserId();
    this.buy_post_data.userName = this.globalService.getUserName();
    this.tableService.getTableData$(this.pageIndex, this.pageSize, this.date).subscribe(
      data => {
        console.log(data);
        this.total = data.totalElements;
        this.listOfAssetsData = data.content;
  ...
  listofAssetsDataColumns: ColumnItem[] = [
    ...
    },
    {
      ...
      filterFn: (a: any, b: any) => {
        if (a === 'Earn') {
          return b.open - b.close <= this.earn_lose_divider
        }
        return b.open  - b.close > this.earn_lose_divider
      }
    },
