...
export class BondTableComponent {
  ...
  onChange(result: Date): void {
    ...
    this.tableService.getBondTableData$(this.pageIndex, this.pageSize, dateString).subscribe(
      data => {
        console.log(data);
  ...
  handleOk(): void {
    ...
    this.buy_post_data.productCode = this.selected_data.bondCode;
    this.buy_post_data.productType = 'BOND';
    ...
    this.postBuyBondData(this.buy_post_data);
    setTimeout(() => {
      this.isVisible = false;
      this.isOkLoading = false;
      this.route.navigate(['/dashboard/operation/sell_bond']);
    }, 3000);
  }
  ...
  public postBuyBondData(data: SellPostData){
    this.tableService.postUserBuyBondData$(data).subscribe(
      (x) => {
        this.loading = true;
        console.log(x);
        // this.getHostData(1, this.date);
        this.tableService.getBondTableData$(this.pageIndex, this.pageSize, data.transactionDate).subscribe(
          data => {
            console.log(data);
            this.total = data.totalElements;
            this.listOfAssetsData = data.content;
  ...
  ngOnInit(): void {
    ...
    this.buy_post_data.userId = this.globalService.getUserId();
    this.buy_post_data.userName = this.globalService.getUserName();
    this.tableService.getBondTableData$(this.pageIndex, this.pageSize, this.date).subscribe(
      data => {
        console.log(data);
        this.total = data.totalElements;
        this.listOfAssetsData = data.content;
  ...
  onQueryParamsChange(params: NzTableQueryParams): void {
    ...
    this.loading = true;
    this.tableService.getBondTableData$(pageIndex, pageSize, this.date).subscribe(
      data => {
        console.log(data);
        this.total = data.totalElements;
        this.listOfAssetsData = data.content;
